...
const concepts = {
  ...
  'provider-comparison': {
    ...
    content: [
      ...
      ``,
      `${pc.dim('Pricing:')} $0.02-$0.12 per million tokens (embedding), $0.02-$0.05/M (reranking)`,
      ``,
      `${pc.bold('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━')}`,
      ``,
  ...
  sessions: {
    ...
    content: [
      `${pc.cyan('Chat sessions')} let you have persistent conversations that survive between CLI`,
      `invocations. Every time you run ${pc.cyan('vai chat')}, a new session is created automatically`,
      `when MongoDB is available.`,
      ``,
      `${pc.bold('Starting a session')}`,
      `Sessions start automatically when MongoDB is reachable. Each session gets a`,
      `unique ID — the first 8 characters are shown in your chat prompt so you can`,
      `identify which session you're in.`,
      ``,
      `${pc.bold('Resuming a session')}`,
      `Use ${pc.cyan('--session <id>')} to pick up where you left off. The CLI loads the full`,
      `conversation history and shows the turn count so you know how far in you are.`,
      `You only need to provide enough of the ID to be unique (the first 8 chars work).`,
      ``,
      `${pc.bold('Listing sessions')}`,
      `Run ${pc.cyan('vai chat --list')} to see your recent sessions. Each entry shows the session`,
      `status, turn count, and date. Add ${pc.cyan('--all')} to include archived sessions in the`,
      `listing.`,
      ``,
      `${pc.bold('Session lifecycle')}`,
      ...
      ``,
      `You can archive a session manually with the ${pc.cyan('/archive')} slash command during chat.`,
      ``,
      `${pc.bold('MongoDB fallback')}`,
      `When MongoDB is unavailable, sessions run entirely in-memory. Your conversation`,
      `still works, but history is lost when you exit. Use ${pc.cyan('--no-history')} to explicitly`,
      `opt into this mode.`,
    ].join('\n'),
  ...
  'memory-strategies': {
    ...
    content: [
      `${pc.cyan('Memory strategies')} control how conversation history is managed to fit within the`,
      `LLM's context window. As conversations grow, not all turns can be sent to the`,
      `model — the memory strategy decides what to keep, summarize, or discard.`,
      ``,
      `${pc.bold('Token budget allocation')}`,
      `The available context window is divided into:`,
      `  ${pc.dim('system prompt')} + ${pc.dim('context docs')} + ${pc.dim('current message')} + ${pc.dim('response reserve')} = total budget`,
      `Whatever remains after these reservations is your ${pc.cyan('history budget')} — the space`,
      `available for past conversation turns.`,
      ``,
      `${pc.bold('sliding_window')} ${pc.dim('(default)')}`,
      `Keeps the most recent turns that fit within the history budget. No LLM calls`,
      `are needed — it simply drops the oldest turns first. This is the fastest`,
      `strategy and works well for short conversations or when only recent context`,
      `matters.`,
      ``,
      `${pc.bold('summarization')}`,
      `Compresses older turns into LLM-generated summaries. Uses a 60/40 split:`,
      `recent turns are kept verbatim (60% of budget) while older turns are`,
      `summarized (40% of budget). Best for long conversations where early context`,
      `matters — the model retains awareness of the full conversation arc.`,
      ``,
      `${pc.bold('hierarchical')}`,
      `Combines recent turns + summaries + cross-session recall from past`,
      `conversations. This is the most context-aware strategy — it surfaces relevant`,
      `information from previous sessions using Voyage AI asymmetric embedding.`,
      `Best when working across multiple sessions on related topics.`,
      ``,
      `${pc.bold('How to select a strategy')}`,
      `Use ${pc.cyan('--memory-strategy <name>')} when starting a chat, or type ${pc.cyan('/memory')} during`,
      `a session to see the current strategy, token budget breakdown, and utilization.`,
    ].join('\n'),
    links: ['https://docs.voyageai.com/'],
    tryIt: [
      'vai chat --memory-strategy sliding_window',
  ...
  'cross-session-recall': {
    ...
    content: [
      `${pc.cyan('Cross-session recall')} surfaces relevant context from past conversations when`,
      `you start or resume a chat session. Instead of starting fresh every time, the`,
      `CLI can "remember" what you discussed before.`,
      ``,
      `${pc.bold('How it works')}`,
      `When a session is archived, its conversation is compressed into a summary.`,
      `That summary is embedded using ${pc.cyan('Voyage AI asymmetric embedding')} and stored`,
      `alongside the session record. When a new session begins (or an existing one`,
      `is resumed), relevant past summaries are retrieved and injected as context.`,
      ``,
      `${pc.bold('Asymmetric approach')}`,
      `Cross-session recall uses two different models from the Voyage 4 family:`,
      `  ${pc.cyan('voyage-4-large')}  for indexing session summaries (maximum accuracy)`,
      `  ${pc.cyan('voyage-4-lite')}   for querying at runtime (speed and cost efficiency)`,
      ``,
      `This works because all Voyage 4 series models share the ${pc.cyan('same embedding space')}`,
      `— vectors from voyage-4-large and voyage-4-lite are directly comparable,`,
      `so you can mix models without losing retrieval quality.`,
      ``,
      `${pc.bold('When it activates')}`,
      `Cross-session recall is used automatically when you select the`,
      `${pc.cyan('hierarchical')} memory strategy. The other strategies (sliding_window and`,
      `summarization) operate within a single session only.`,
      ``,
      `${pc.bold('Practical benefit')}`,
      `You can "pick up where you left off" even across topic boundaries. If you`,
      `discussed database indexing strategies last week and start a new session`,
      `about query performance, the hierarchical strategy will surface those`,
      `earlier insights automatically.`,
    ].join('\n'),
    links: ['https://docs.voyageai.com/'],
    tryIt: [
      'vai chat --memory-strategy hierarchical',
      'vai explain memory-strategies',
    ],
  },
};
...
const aliases = {
  ...
  // Memory strategies aliases
  memory: 'memory-strategies',
  'memory-strategy': 'memory-strategies',
  strategies: 'memory-strategies',
  'sliding-window': 'memory-strategies',
  sliding_window: 'memory-strategies',
  // Cross-session recall aliases
  'cross-session': 'cross-session-recall',
  recall: 'cross-session-recall',
  'session-recall': 'cross-session-recall',
};
