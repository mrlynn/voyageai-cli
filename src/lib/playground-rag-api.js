...
function registerRAGAPI(server, context) {
   ...
   */
  server.on('request', async (req, res) => {
    const match = req.url.match(/^\/api\/rag\/kb\/([^/]+)$/);
    if (req.method === 'GET' && match) {
      try {
        const kbName = decodeURIComponent(match[1]);
        const { client, collection: kbsCollection } = await getMongoCollection(RAG_DB, KBS_COLLECTION);
        const kb = await kbsCollection.findOne({ name: kbName });
        client.close();
   ...
   */
  server.on('request', async (req, res) => {
    const match = req.url.match(/^\/api\/rag\/docs\/([^/]+)\/([^/]+)$/);
    if (req.method === 'DELETE' && match) {
      try {
        const kbName = decodeURIComponent(match[1]);
